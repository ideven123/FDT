...
from advertorch.attacks import GradientSignAttack, LinfBasicIterativeAttack, LinfPGDAttack, CarliniWagnerL2Attack,LinfMomentumIterativeAttack
    ...
    if args.convergence_check:
        ...
        for eps in tqdm(eps_list, desc='PGD eps', leave=True, position=0):            
            ...
            for i in tqdm(range(args.random_start), desc='Random Start', leave=False, position=1):
                ...
                # ensemble, confidence=0.1, max_iterations=100, clip_min=0., clip_max=1.,
                # targeted=False, num_classes=10, binary_search_steps=1,initial_const=0.1)
                adversary = LinfMomentumIterativeAttack(
                ensemble, loss_fn=loss_fn, eps=eps,
                nb_iter=args.steps, eps_iter=eps / 5, clip_min=0., clip_max=1.,
                targeted=False)
                _, label, pred, advpred = attack_whole_dataset(adversary, test_iter, device="cuda")
